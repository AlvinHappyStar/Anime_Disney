...
export default function FileDropZone({ type, uploadType }: any) {
  ...
  const handleSubmit: IDropzoneProps["onSubmit"] = async (allFiles) => {
    ...
    allFiles.forEach((f) => {
      ...
      if (uploadType === "gif") payload.type = gifType;
      if (uploadType === "music") payload.type = musicType;
      if (uploadType === "video") payload.type = videoType;
      if (uploadType === "picture") payload.type = pictureType;
    ...
    if (uploadType === "gif") GifService.addGif(arr, dispatch);
    if (uploadType === "music") MusicService.addMusic(arr, dispatch);
    if (uploadType === "video") VideoService.addVideo(arr, dispatch);
    if (uploadType === "picture") PictureService.addPicture(arr, dispatch);
    if (tab === "user") {
